...
local posXprev, posYprev, posZprev = 0, 0, 0

-- set UI scaling
local scale = 1.25
-- 4x = 0.997 | 5x = 1.25 | 6x = 1.5

-- warning about window scaling
console.log("Default window size: 4x, you can change it by changing the \"scale\" variable\n")

-- Function to insert a value in a buffer table with a size limit and calculate average of it
local function buffer_average(value, buffer, size)
  table.insert(buffer, 1, value) -- Insert the new value in the buffer table, assuming buffer table is 1-index
  if #buffer == size then buffer[size] = nil end -- Chop last value in buffer if reached buffer limit size
  local average = 0
  for i = 1, #buffer do  -- Calculate average, assuming buffer table is 1-index
